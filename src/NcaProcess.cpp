...
nstool::NcaProcess::NcaProcess()
    : mModuleName("nstool::NcaProcess"), mFile(), mOutputFile(), mCliOutputMode(), mKeyCfg(), mVerify(false),
      mBaseNcaPath(), mFileSystem(), mFsProcess()
{
}
...
void nstool::NcaProcess::generateNcaBodyEncryptionKeys()
{
    ...
    for (size_t i = 0; i < mHdr.getKeyArea().size(); i++)
    {
        if (mHdr.getKeyArea()[i] != zero_aesctr_key)
        {
            ...
            // key[0-3]
            if (i < 4 && mKeyCfg.nca_key_area_encryption_key[keak_index].find(masterkey_rev) !=
                         ...
                         mKeyCfg.nca_key_area_encryption_key_hw[keak_index].end())
            {
                ...
                pie::hac::AesKeygen::generateKey(
                    ...
                    mKeyCfg.nca_key_area_encryption_key_hw[keak_index][masterkey_rev].data());
            }
            else
            {
                kak.decrypted = false;
    ...
    else
    {
        ...
        {
            if (mContentKey.kak_list[i].index == pie::hac::nca::KeyBankIndex_AesCtr &&
...
nstool::NcaProcess nstool::NcaProcess::readBaseNCA()
{
    ...
    std::shared_ptr<tc::io::IStream> base_stream = std::make_shared<tc::io::FileStream>(
        tc::io::FileStream(mBaseNcaPath.get(), tc::io::FileMode::Open, tc::io::FileAccess::Read));
        ...
        try
        {
            ...
            else
            {
                ...
                else if (info.enc_type == pie::hac::nca::EncryptionType_AesCtr)
                {
                    ...
                    info.decrypt_reader = std::make_shared<tc::crypto::Aes128CtrEncryptedStream>(
                        tc::crypto::Aes128CtrEncryptedStream(info.raw_reader, partition_key, partition_ctr));
                }
                else if (info.enc_type == pie::hac::nca::EncryptionType_AesCtrEx)
                {
                    ...
                    // TODO: see if AesCtrEx encryption can just be for creating the transparent decryption, with
                    // IndirectStorage IStream construction being done after decryption but before hash layer processing
                    ...
                    {
                        if (partition_base.format_type == pie::hac::nca::FormatType::FormatType_RomFs &&
                            partition_base.raw_reader != nullptr)
                        {
                    ...
                    // create decryption stream
                    info.decrypt_reader = std::make_shared<pie::hac::BKTREncryptedStream>(pie::hac::BKTREncryptedStream(
                        info.raw_reader, partition_key, partition_ctr, fs_header.patch_info, base_reader));
                }
            ...
            case (pie::hac::nca::HashType_HierarchicalSha256):
                info.reader = std::make_shared<pie::hac::HierarchicalSha256Stream>(
                    pie::hac::HierarchicalSha256Stream(info.decrypt_reader, info.hierarchicalsha256_hdr));
                break;
            case (pie::hac::nca::HashType_HierarchicalIntegrity):
                info.reader = std::make_shared<pie::hac::HierarchicalIntegrityStream>(
...
void nstool::NcaProcess::validateNcaSignatures()
{
    ...
    if (mKeyCfg.nca_header_sign0_key.find(mHdr.getSignatureKeyGeneration()) != mKeyCfg.nca_header_sign0_key.end())
    {
            ...
            false)
        {
            ...
            r.push("events", nlohmann::json{{"severity", "warn"}, {"message", "NCA Header Main Signature: FAIL."}});
        }
    }
    else
    {
    ...
    if (mHdr.getContentType() == pie::hac::nca::ContentType_Program)
    {
        try
        {
            if (mPartitions[pie::hac::nca::ProgramContentPartitionIndex_Code].format_type ==
                pie::hac::nca::FormatType_PartitionFs)
            {
                if (mPartitions[pie::hac::nca::ProgramContentPartitionIndex_Code].fs_reader != nullptr)
                {
                    ...
                    try
                    {
                        mPartitions[pie::hac::nca::ProgramContentPartitionIndex_Code].fs_reader->openFile(
                            tc::io::Path(kNpdmExefsPath), tc::io::FileMode::Open, tc::io::FileAccess::Read, npdm_file);
                    }
                    ...
                    if (tc::crypto::VerifyRsa2048PssSha2256(
                            mHdrBlock.signature_acid.data(), mHdrHash.data(),
                            npdm.getMeta().getAccessControlInfoDesc().getContentArchiveHeaderSignature2Key()) == false)
                    {
...
void nstool::NcaProcess::processPartitions()
{
    ...
    for (size_t i = 0; i < mHdr.getPartitionEntryList().size(); i++)
    {
        ...
        if (partition.fs_reader == nullptr)
        {
            ...
            if (partition.fail_reason.empty() == false)
            {
                ...
                r.push("events", nlohmann::json{{"severity", "warn"}, {"message", partition.fail_reason}});
            }
            continue;
        }
        ...
        {
                mount_point_name =
        pie::hac::ContentArchiveUtil::getProgramContentParititionIndexAsString((pie::hac::nca::ProgramContentPartitionIndex)index);
        }
        else
        */
    ...
    std::shared_ptr<tc::io::IFileSystem> nca_fs =
        std::make_shared<tc::io::VirtualFileSystem>(tc::io::VirtualFileSystem(fs_snapshot));
