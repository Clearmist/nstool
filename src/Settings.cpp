...
class DeprecatedOptionHandler : public tc::cli::OptionParser::IOptionHandler
{
  public:
    DeprecatedOptionHandler(const std::string &warn_message, const std::vector<std::string> &opts)
        : mWarnMessage(warn_message), mOptStrings(opts), mOptRegex()
    {
    }
    ...
    void processOption(const std::string &option, const std::vector<std::string> &params)
    {
        if (params.size() != 0)
        {
            throw tc::ArgumentOutOfRangeException(
                fmt::format("Option \"{:s}\" is a flag, that takes no parameters.", option));
        }
...
class SingleParamAesKeyOptionHandler : public tc::cli::OptionParser::IOptionHandler
{
  public:
    ...
    void processOption(const std::string &option, const std::vector<std::string> &params)
    {
        ...
        {
            throw tc::ArgumentOutOfRangeException(fmt::format(
                "Option: \"{:s}\", requires an AES128 key as the parameter (must be 32 hex chars).", option));
        }
...
class FileTypeOptionHandler : public tc::cli::OptionParser::IOptionHandler
{
  public:
    ...
    void processOption(const std::string &option, const std::vector<std::string> &params)
    {
        ...
        if (params[0] == "gc" || params[0] == "gamecard" || params[0] == "xci" || params[0] == "xcie" ||
            params[0] == "xcir")
        {
...
class SingleFileExtractionHandler : public tc::cli::OptionParser::IOptionHandler
{
  public:
    ...
    void processOption(const std::string &option, const std::vector<std::string> &params)
    {
        ...
        {
            throw tc::ArgumentOutOfRangeException(fmt::format(
                "Option \"{:s}\" requires a parameter. Give the name and extension of the file you want to extract.",
                option));
        }
    ...
    void processOption(const std::string &option, const std::vector<std::string> &params)
    {
        ...
        else
        {
            throw tc::ArgumentException(
                fmt::format("Instruction type \"{}\" unrecognised. Try \"32bit\" or \"64bit\"", params[0]));
        }
    }
...
class ExtractDataPathOptionHandler : public tc::cli::OptionParser::IOptionHandler
{
  public:
    ExtractDataPathOptionHandler(std::vector<nstool::ExtractJob> &jobs, const std::vector<std::string> &opts)
        : mJobs(jobs), mOptStrings(opts), mOptRegex()
    {
    }
    ...
    void processOption(const std::string &option, const std::vector<std::string> &params)
    {
        ...
        {
            throw tc::ArgumentOutOfRangeException(fmt::format(
                "Option \"{:s}\" requires parameters in the format \"[<internal path>] <extract path>\".", option));
        }
    }
...
class CustomExtractDataPathOptionHandler : public tc::cli::OptionParser::IOptionHandler
{
  public:
    ...
    void processOption(const std::string &option, const std::vector<std::string> &params)
    {
        ...
        {
            message =
                message + fmt::format("Consider using \"-x {:s} {:s}\" instead.", mCustomPath.to_string(), params[0]);
        }
...
nstool::SettingsInitializer::SettingsInitializer(const std::vector<std::string> &args)
    : Settings(), mModuleLabel("nstool::SettingsInitializer"), mShowLayout(false), mShowKeydata(false), mVerbose(false),
      mMachineReadable(false), mNcaEncryptedContentKey(), mNcaContentKey(), mTikPathList(), mCertPath()
{
...
void nstool::SettingsInitializer::parse_args(const std::vector<std::string> &args)
{
    ...
    // get option flags
    opts.registerOptionHandler(
        std::shared_ptr<FlagOptionHandler>(new FlagOptionHandler(mShowLayout, {"--showlayout"})));
    opts.registerOptionHandler(std::shared_ptr<FlagOptionHandler>(new FlagOptionHandler(mShowKeydata, {"--showkeys"})));
    opts.registerOptionHandler(
        std::shared_ptr<FlagOptionHandler>(new FlagOptionHandler(mVerbose, {"-v", "--verbose"})));
    opts.registerOptionHandler(std::shared_ptr<FlagOptionHandler>(new FlagOptionHandler(mMachineReadable, {"--json"})));
    opts.registerOptionHandler(
        std::shared_ptr<FlagOptionHandler>(new FlagOptionHandler(opt.verify, {"-y", "--verify"})));
    ...
    opts.registerOptionHandler(
        std::shared_ptr<FileTypeOptionHandler>(new FileTypeOptionHandler(infile.filetype, {"-t", "--type"})));
    opts.registerOptionHandler(
        std::shared_ptr<SingleFileExtractionHandler>(new SingleFileExtractionHandler(outfile.filename, {"--file"})));

    // get user-provided keydata
    opts.registerOptionHandler(std::shared_ptr<SingleParamPathOptionHandler>(
        new SingleParamPathOptionHandler(mKeysetPath, {"-k", "--keyset"})));
    opts.registerOptionHandler(std::shared_ptr<SingleParamAesKeyOptionHandler>(
        new SingleParamAesKeyOptionHandler(mNcaEncryptedContentKey, {"--titlekey"})));
    opts.registerOptionHandler(std::shared_ptr<SingleParamAesKeyOptionHandler>(
        new SingleParamAesKeyOptionHandler(mNcaContentKey, {"--contentkey", "--bodykey"})));
    opts.registerOptionHandler(std::shared_ptr<SingleParamPathArrayOptionHandler>(
        new SingleParamPathArrayOptionHandler(mTikPathList, {"--tik"})));
    opts.registerOptionHandler(
        std::shared_ptr<SingleParamPathOptionHandler>(new SingleParamPathOptionHandler(mCertPath, {"--cert"})));

    // code options
    opts.registerOptionHandler(std::shared_ptr<FlagOptionHandler>(new FlagOptionHandler(code.list_api, {"--listapi"})));
    opts.registerOptionHandler(
        std::shared_ptr<FlagOptionHandler>(new FlagOptionHandler(code.list_symbols, {"--listsym"})));
    opts.registerOptionHandler(std::shared_ptr<InstructionTypeOptionHandler>(
        new InstructionTypeOptionHandler(code.is_64bit_instruction, {"--insttype"})));

    // fs options
    opts.registerOptionHandler(
        std::shared_ptr<FlagOptionHandler>(new FlagOptionHandler(fs.show_fs_tree, {"--fstree", "--listfs"})));
    opts.registerOptionHandler(std::shared_ptr<ExtractDataPathOptionHandler>(
        new ExtractDataPathOptionHandler(fs.extract_jobs, {"-x", "--extract"})));
    opts.registerOptionHandler(std::shared_ptr<CustomExtractDataPathOptionHandler>(
        new CustomExtractDataPathOptionHandler(fs.extract_jobs, {"--fsdir"}, tc::io::Path("/"))));

    // xci options
    opts.registerOptionHandler(std::shared_ptr<CustomExtractDataPathOptionHandler>(
        new CustomExtractDataPathOptionHandler(fs.extract_jobs, {"--update"}, tc::io::Path("/update/"))));
    opts.registerOptionHandler(std::shared_ptr<CustomExtractDataPathOptionHandler>(
        new CustomExtractDataPathOptionHandler(fs.extract_jobs, {"--normal"}, tc::io::Path("/normal/"))));
    opts.registerOptionHandler(std::shared_ptr<CustomExtractDataPathOptionHandler>(
        new CustomExtractDataPathOptionHandler(fs.extract_jobs, {"--secure"}, tc::io::Path("/secure/"))));
    opts.registerOptionHandler(std::shared_ptr<CustomExtractDataPathOptionHandler>(
        new CustomExtractDataPathOptionHandler(fs.extract_jobs, {"--logo"}, tc::io::Path("/logo/"))));

    // nca options
    opts.registerOptionHandler(std::shared_ptr<CustomExtractDataPathOptionHandler>(
        new CustomExtractDataPathOptionHandler(fs.extract_jobs, {"--part0"}, tc::io::Path("/0/"))));
    opts.registerOptionHandler(std::shared_ptr<CustomExtractDataPathOptionHandler>(
        new CustomExtractDataPathOptionHandler(fs.extract_jobs, {"--part1"}, tc::io::Path("/1/"))));
    opts.registerOptionHandler(std::shared_ptr<CustomExtractDataPathOptionHandler>(
        new CustomExtractDataPathOptionHandler(fs.extract_jobs, {"--part2"}, tc::io::Path("/2/"))));
    opts.registerOptionHandler(std::shared_ptr<CustomExtractDataPathOptionHandler>(
        new CustomExtractDataPathOptionHandler(fs.extract_jobs, {"--part3"}, tc::io::Path("/3/"))));

    opts.registerOptionHandler(std::shared_ptr<SingleParamPathOptionHandler>(
        new SingleParamPathOptionHandler(nca.base_nca_path, {"--basenca"})));

    // kip options
    opts.registerOptionHandler(std::shared_ptr<SingleParamPathOptionHandler>(
        new SingleParamPathOptionHandler(kip.extract_path, {"--kipdir"})));

    // aset options
    opts.registerOptionHandler(std::shared_ptr<SingleParamPathOptionHandler>(
        new SingleParamPathOptionHandler(aset.icon_extract_path, {"--icon"})));
    opts.registerOptionHandler(std::shared_ptr<SingleParamPathOptionHandler>(
        new SingleParamPathOptionHandler(aset.nacp_extract_path, {"--nacp"})));
...
void nstool::SettingsInitializer::determine_filetype()
{
    ...
    auto file = tc::io::StreamSource(std::make_shared<tc::io::FileStream>(
        tc::io::FileStream(infile.path.get(), tc::io::FileMode::Open, tc::io::FileAccess::Read)));
...
void nstool::SettingsInitializer::usage_text() const
{
    ...
    fmt::print("      -t, --type      Specify input file type. [xci, pfs, romfs, nca, meta, cnmt, nso, nro, ini, kip, "
               "nacp, aset, cert, tik]\n");
    ...
    fmt::print("      --json          Converts output to machine readable JSON. Automatically turns on all other "
               "output options.\n");
...
void nstool::SettingsInitializer::dump_keys() const
{
    ...
    {
        for (auto itr = opt.keybag.nca_key_area_encryption_key[kaek_index].begin();
    ...
    {
        for (auto itr = opt.keybag.nca_key_area_encryption_key_hw[kaek_index].begin();
...
bool nstool::SettingsInitializer::determineValidNcaFromSample(const tc::ByteData &sample) const
{
    ...
    if (opt.keybag.nca_header_key.isNull())
    {
        ...
        r.push("events", nlohmann::json{{"severity", "warn"}, {"message", "Failed to load NCA Header Key."}});
    ...
    if (hdr->st_magic.unwrap() != pie::hac::nca::kNca2StructMagic &&
        hdr->st_magic.unwrap() != pie::hac::nca::kNca3StructMagic)
    {
        return false;
    }
...
bool nstool::SettingsInitializer::determineValidCnmtFromSample(const tc::ByteData &sample) const
{
    ...
    size_t minimum_size = sizeof(pie::hac::sContentMetaHeader) + data->exhdr_size.unwrap() +
                          data->content_count.unwrap() * sizeof(pie::hac::sContentInfo) +
                          data->content_meta_count.unwrap() * sizeof(pie::hac::sContentMetaInfo) +
                          pie::hac::cnmt::kDigestLen;
    ...
    if (data->exhdr_size.unwrap() > 0)
    {
        ...
        {
            const pie::hac::sPatchMetaExtendedHeader *meta =
                (const pie::hac::sPatchMetaExtendedHeader *)(sample.data() + sizeof(pie::hac::sContentMetaHeader));
        ...
        {
            const pie::hac::sDeltaMetaExtendedHeader *meta =
                (const pie::hac::sDeltaMetaExtendedHeader *)(sample.data() + sizeof(pie::hac::sContentMetaHeader));
...
bool nstool::SettingsInitializer::determineValidEsCertFromSample(const tc::ByteData &sample) const
{
    ...
    if (sign.getSignType() != pie::hac::es::sign::SIGN_ID_RSA4096_SHA256 &&
        sign.getSignType() != pie::hac::es::sign::SIGN_ID_RSA2048_SHA256 &&
        sign.getSignType() != pie::hac::es::sign::SIGN_ID_ECDSA240_SHA256)
    {
        return false;
    }
...
bool nstool::SettingsInitializer::determineValidEsTikFromSample(const tc::ByteData &sample) const
{
    ...
    const pie::hac::es::sTicketBody_v2 *body =
        (const pie::hac::es::sTicketBody_v2 *)(sample.data() + sign.getBytes().size());
