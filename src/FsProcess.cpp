...
nstool::FsProcess::FsProcess()
    : mModuleLabel("nstool::FsProcess"), mInputFs(), mFsFormatName(), mProperties(), mShowFsInfo(true),
      mShowFsTree(false), mFsRootLabel(), mExtractJobs(), mOutputFile(), mDataCache(0x10000)
{
}
...
void nstool::FsProcess::setFsFormatName(const std::string &fs_format_name) { mFsFormatName = fs_format_name; }

void nstool::FsProcess::setFsProperties(const std::vector<std::string> &properties) { mProperties = properties; }

void nstool::FsProcess::setShowFsInfo(bool show_fs_info) { mShowFsInfo = show_fs_info; }

void nstool::FsProcess::setShowFsTree(bool show_fs_tree) { mShowFsTree = show_fs_tree; }

void nstool::FsProcess::setFsRootLabel(const std::string &root_label) { mFsRootLabel = root_label; }
...
void nstool::FsProcess::setExtractFile(std::string outputFile) { mOutputFile = outputFile; }

void nstool::FsProcess::printFs()
{
    Report &r = get_report();

    r.text(fmt::format("[{:s}/Tree]", (mFsFormatName.isSet() ? mFsFormatName.get() : "FileSystem")));
...
void nstool::FsProcess::extractFs()
{
    ...
    r.text(fmt::format("[{:s}/Extract]", (mFsFormatName.isSet() ? mFsFormatName.get() : "FileSystem")));
    ...
    for (auto itr = mExtractJobs.begin(); itr != mExtractJobs.end(); itr++)
    {
        ...
        try
        {
            ...
            std::shared_ptr<tc::io::IFileSystem> local_fs =
                std::make_shared<tc::io::LocalFileSystem>(tc::io::LocalFileSystem());
            ...
            // method: try getDirectoryListing(itr->extract_path), if this is does not throw, then we can be sure this
            // is a valid path to a directory, file_extract_path = itr->extract_path + itr->virtual_path.back()
            ...
            // case: the extract_path up until the last element is a valid path to an existing directory, but the full
            // path specifies neither a directory or a file behaviour: treat extract_path as the intended location to
            // write the extracted file (the original filename is not preserved, instead specified by the user in the
            // final element of the extract path) method: since this checks n-1 elements, it implies a path with more
            // than one element, so that must be accounted for, as relative paths are valid and single element paths
            // aren't always root
            ...
            try
            {
                ...
                r.text(fmt::format("Saving {:s} as {:s}...", itr->virtual_path.to_string(),
                                   itr->extract_path.to_string()));
                r.push("events",
                       nlohmann::json{{"severity", "info"},
                                      {"message", fmt::format("Extracting {:s} as {:s}", itr->virtual_path.to_string(),
                                                              itr->extract_path.to_string())}});
            ...
            r.text(fmt::format("[WARNING] Extract path was invalid, and was skipped: {:s}",
                               itr->extract_path.to_string()));
            r.push("events", nlohmann::json{{"severity", "warn"},
                                            {"message", fmt::format("Extract path was invalid, and was skipped: {:s}",
                                                                    itr->extract_path.to_string())}});
            continue;
        }
...
void nstool::FsProcess::visitDir(const tc::io::Path &v_path, const tc::io::Path &l_path, bool extract_fs, bool print_fs)
{
    ...
    if (print_fs)
    {
        ...
        r.text(fmt::format(padded, ((v_path.size() == 1) ? (rootLabel + ":") : v_path.back())));
    }
    ...
    for (auto itr = info.file_list.begin(); itr != info.file_list.end(); itr++)
    {
        ...
        if (extract_fs && (mOutputFile == "" || (mOutputFile == *itr)))
        {
            ...
            for (int64_t remaining_data = in_stream->length(); remaining_data > 0;)
            {
                ...
                if (cache_read_len == 0)
                {
                    throw tc::io::IOException(mModuleLabel,
                                              fmt::format("Failed to read from {:s} file.",
                                                          (mFsFormatName.isSet() ? (mFsFormatName.get() + " ") : "")));
                }
    ...
    {
        // When traversing each directory append the directory to the local path only if we're not looking to extract a
        // single file.
