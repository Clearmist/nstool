...
void nstool::CnmtProcess::displayCnmt()
{
	...
	r.text(fmt::format("  Version:               {:s} (v{:d})", pie::hac::ContentMetaUtil::getVersionAsString(mCnmt.getTitleVersion()), mCnmt.getTitleVersion()));
	r.text(fmt::format("  Type:                  {:s} ({:d})", pie::hac::ContentMetaUtil::getContentMetaTypeAsString(mCnmt.getContentMetaType()), (uint32_t)mCnmt.getContentMetaType()));
	...
	r.text(fmt::format("  StorageId:             {:s} ({:d})", pie::hac::ContentMetaUtil::getStorageIdAsString(mCnmt.getStorageId()), (uint32_t)mCnmt.getStorageId()));
	r.text(fmt::format("  ContentInstallType:    {:s} ({:d})", pie::hac::ContentMetaUtil::getContentInstallTypeAsString(mCnmt.getContentInstallType()),(uint32_t)mCnmt.getContentInstallType()));
	r.text(fmt::format("  RequiredDownloadSystemVersion: {:s} (v{:d})", pie::hac::ContentMetaUtil::getVersionAsString(mCnmt.getRequiredDownloadSystemVersion()), mCnmt.getRequiredDownloadSystemVersion()));

	r.set("data.contentMeta.storageId", nlohmann::json{
		{"string", pie::hac::ContentMetaUtil::getStorageIdAsString(mCnmt.getStorageId())},
		{"int", mCnmt.getStorageId()}
	});
	r.set("data.contentMeta.contentInstallType", nlohmann::json{
		{"string", pie::hac::ContentMetaUtil::getContentInstallTypeAsString(mCnmt.getContentInstallType())},
		{"int", mCnmt.getContentInstallType()}
	});
	r.set("data.contentMeta.requiredDownloadSystemVersion", nlohmann::json{
	...
	switch(mCnmt.getContentMetaType())
	{
		case (pie::hac::cnmt::ContentMetaType_Application):
			...
			r.text(fmt::format("    RequiredApplicationVersion: {:s} (v{:d})", pie::hac::ContentMetaUtil::getVersionAsString(mCnmt.getApplicationMetaExtendedHeader().getRequiredApplicationVersion()), mCnmt.getApplicationMetaExtendedHeader().getRequiredApplicationVersion()));
			r.text(fmt::format("    RequiredSystemVersion:      {:s} (v{:d})", pie::hac::ContentMetaUtil::getVersionAsString(mCnmt.getApplicationMetaExtendedHeader().getRequiredSystemVersion()), mCnmt.getApplicationMetaExtendedHeader().getRequiredSystemVersion()));
			r.text(fmt::format("    PatchId:                    0x{:016x}", mCnmt.getApplicationMetaExtendedHeader().getPatchId()));

			r.set("data.contentMeta.applicationExtendedHeader.requiredApplicationVersion", nlohmann::json{
				{"string", pie::hac::ContentMetaUtil::getVersionAsString(mCnmt.getApplicationMetaExtendedHeader().getRequiredApplicationVersion())},
				{"int", mCnmt.getApplicationMetaExtendedHeader().getRequiredApplicationVersion()}
			});
			r.set("data.contentMeta.applicationExtendedHeader.requiredSystemVersion", nlohmann::json{
				{"string", pie::hac::ContentMetaUtil::getVersionAsString(mCnmt.getApplicationMetaExtendedHeader().getRequiredSystemVersion())},
				{"int", mCnmt.getApplicationMetaExtendedHeader().getRequiredSystemVersion()}
			});
			r.set("data.contentMeta.applicationExtendedHeader.patchId", fmt::format("{:016x}", mCnmt.getApplicationMetaExtendedHeader().getPatchId()));
			break;
		case (pie::hac::cnmt::ContentMetaType_Patch):
			r.text("  PatchMetaExtendedHeader:");
			r.text(fmt::format("    RequiredSystemVersion: {:s} (v{:d})", pie::hac::ContentMetaUtil::getVersionAsString(mCnmt.getPatchMetaExtendedHeader().getRequiredSystemVersion()), mCnmt.getPatchMetaExtendedHeader().getRequiredSystemVersion()));
			r.text(fmt::format("    ApplicationId:         0x{:016x}", mCnmt.getPatchMetaExtendedHeader().getApplicationId()));

			r.set("data.contentMeta.patchMetaExtendedHeader.requiredSystemVersion", nlohmann::json{
				{"string", pie::hac::ContentMetaUtil::getVersionAsString(mCnmt.getPatchMetaExtendedHeader().getRequiredSystemVersion())},
				{"int", mCnmt.getPatchMetaExtendedHeader().getRequiredSystemVersion()}
			});
			r.set("data.contentMeta.patchMetaExtendedHeader.applicationId", fmt::format("{:016x}", mCnmt.getPatchMetaExtendedHeader().getApplicationId()));
			break;
		case (pie::hac::cnmt::ContentMetaType_AddOnContent):
			r.text("  AddOnContentMetaExtendedHeader:");
			r.text(fmt::format("    RequiredApplicationVersion: {:s} (v{:d})", pie::hac::ContentMetaUtil::getVersionAsString(mCnmt.getAddOnContentMetaExtendedHeader().getRequiredApplicationVersion()), mCnmt.getAddOnContentMetaExtendedHeader().getRequiredApplicationVersion()));
			r.text(fmt::format("    ApplicationId:         0x{:016x}", mCnmt.getAddOnContentMetaExtendedHeader().getApplicationId()));

			r.set("data.contentMeta.addOnContentMetaExtendedHeader.requiredApplicationVersion", nlohmann::json{
				{"string", pie::hac::ContentMetaUtil::getVersionAsString(mCnmt.getAddOnContentMetaExtendedHeader().getRequiredApplicationVersion())},
				{"int", mCnmt.getAddOnContentMetaExtendedHeader().getRequiredApplicationVersion()}
			});
			r.set("data.contentMeta.addOnContentMetaExtendedHeader.applicationId", fmt::format("{:016x}", mCnmt.getAddOnContentMetaExtendedHeader().getApplicationId()));
			break;
		case (pie::hac::cnmt::ContentMetaType_Delta):
			r.text("  DeltaMetaExtendedHeader:");
			r.text(fmt::format("    ApplicationId:         0x{:016x}", mCnmt.getDeltaMetaExtendedHeader().getApplicationId()));

			r.set("data.contentMeta.deltaMetaExtendedHeader.applicationId", fmt::format("{:016x}", mCnmt.getDeltaMetaExtendedHeader().getApplicationId()));
			break;
		default:
			break;
	}
	...
	if (mCnmt.getContentInfo().size() > 0)
	{
		...
		for (size_t i = 0; i < mCnmt.getContentInfo().size(); i++)
		{
			...
			r.text(fmt::format("      Type:         {:s} ({:d})", pie::hac::ContentMetaUtil::getContentTypeAsString(info.getContentType()), (uint32_t)info.getContentType()));
			r.text(fmt::format("      Id:           {:s}", tc::cli::FormatUtil::formatBytesAsString(info.getContentId().data(), info.getContentId().size(), false, "")));
			r.text(fmt::format("      Size:         0x{:x}", info.getContentSize()));
			r.text(fmt::format("      Hash:         {:s}", tc::cli::FormatUtil::formatBytesAsString(info.getContentHash().data(), info.getContentHash().size(), false, "")));

			r.push("data.contentMeta.content", nlohmann::json{
				{"type", nlohmann::json{
					{"string", pie::hac::ContentMetaUtil::getContentTypeAsString(info.getContentType())},
					{"int", info.getContentType()}
				}},
				{"id", tc::cli::FormatUtil::formatBytesAsString(info.getContentId().data(), info.getContentId().size(), false, "")},
				{"size", nlohmann::json{
					{"hex", fmt::format("0x{:x}", info.getContentSize())},
					{"int", info.getContentSize()}
				}},
				{"hash", tc::cli::FormatUtil::formatBytesAsString(info.getContentHash().data(), info.getContentHash().size(), false, "")}
			});
		}
	}
	...
	else if (mCnmt.getContentMetaType() == pie::hac::cnmt::ContentMetaType_SystemUpdate && mCnmt.getSystemUpdateMetaExtendedHeader().getExtendedDataSize() != 0)
	{
		...
		r.text(fmt::format("    FormatVersion:         {:d}", mCnmt.getSystemUpdateMetaExtendedData().getFormatVersion()));
		r.text("    FirmwareVariation:");

		r.set("data.contentMeta.systemUpdateMetadata.formatVersion", mCnmt.getSystemUpdateMetaExtendedData().getFormatVersion());
	...
	r.text(fmt::format("  Digest:   {:s}", tc::cli::FormatUtil::formatBytesAsString(mCnmt.getDigest().data(), mCnmt.getDigest().size(), false, "")));
	r.set("data.contentMeta.digest", tc::cli::FormatUtil::formatBytesAsString(mCnmt.getDigest().data(), mCnmt.getDigest().size(), false, ""));
}
...
void nstool::CnmtProcess::displayContentMetaInfoList(const std::vector<pie::hac::ContentMetaInfo>& content_meta_info_list, const std::string& prefix)
{
	for (size_t i = 0; i < content_meta_info_list.size(); i++)
	{
		...
		// TODO: Will this work? It is expecting a string and integer, but receives only an integer.
		fmt::print("{:s}{:d}\n", i);
