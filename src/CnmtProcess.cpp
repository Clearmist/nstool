...
void nstool::CnmtProcess::setInputFile(const std::shared_ptr<tc::io::IStream> &file) { mFile = file; }

void nstool::CnmtProcess::setVerifyMode(bool verify) { mVerify = verify; }

const pie::hac::ContentMeta &nstool::CnmtProcess::getContentMeta() const { return mCnmt; }
...
void nstool::CnmtProcess::displayCnmt()
{
    ...
    r.text(fmt::format("  Version:               {:s} (v{:d})",
                       pie::hac::ContentMetaUtil::getVersionAsString(mCnmt.getTitleVersion()),
                       mCnmt.getTitleVersion()));
    r.text(fmt::format("  Type:                  {:s} ({:d})",
    ...
    r.set("data.contentMeta.version",
          nlohmann::json{{"string", pie::hac::ContentMetaUtil::getVersionAsString(mCnmt.getTitleVersion())},
                         {"int", mCnmt.getTitleVersion()}});
    r.set("data.contentMeta.type",
          nlohmann::json{{"string", pie::hac::ContentMetaUtil::getContentMetaTypeAsString(mCnmt.getContentMetaType())},
    ...
    case (pie::hac::cnmt::ContentMetaType_Application):
        ...
        r.text(fmt::format("    RequiredApplicationVersion: {:s} (v{:d})",
                           pie::hac::ContentMetaUtil::getVersionAsString(
                               mCnmt.getApplicationMetaExtendedHeader().getRequiredApplicationVersion()),
                           mCnmt.getApplicationMetaExtendedHeader().getRequiredApplicationVersion()));
        r.text(fmt::format("    RequiredSystemVersion:      {:s} (v{:d})",
                           pie::hac::ContentMetaUtil::getVersionAsString(
                               mCnmt.getApplicationMetaExtendedHeader().getRequiredSystemVersion()),
                           mCnmt.getApplicationMetaExtendedHeader().getRequiredSystemVersion()));
        r.text(fmt::format("    PatchId:                    0x{:016x}",
                           mCnmt.getApplicationMetaExtendedHeader().getPatchId()));

        r.set("data.contentMeta.applicationExtendedHeader.requiredApplicationVersion",
              nlohmann::json{{"string", pie::hac::ContentMetaUtil::getVersionAsString(
                                            mCnmt.getApplicationMetaExtendedHeader().getRequiredApplicationVersion())},
                             {"int", mCnmt.getApplicationMetaExtendedHeader().getRequiredApplicationVersion()}});
        r.set("data.contentMeta.applicationExtendedHeader.requiredSystemVersion",
              nlohmann::json{{"string", pie::hac::ContentMetaUtil::getVersionAsString(
                                            mCnmt.getApplicationMetaExtendedHeader().getRequiredSystemVersion())},
                             {"int", mCnmt.getApplicationMetaExtendedHeader().getRequiredSystemVersion()}});
        r.set("data.contentMeta.applicationExtendedHeader.patchId",
    ...
    case (pie::hac::cnmt::ContentMetaType_Patch):
        ...
        r.set("data.contentMeta.patchMetaExtendedHeader.requiredSystemVersion",
              nlohmann::json{{"string", pie::hac::ContentMetaUtil::getVersionAsString(
                                            mCnmt.getPatchMetaExtendedHeader().getRequiredSystemVersion())},
                             {"int", mCnmt.getPatchMetaExtendedHeader().getRequiredSystemVersion()}});
        r.set("data.contentMeta.patchMetaExtendedHeader.applicationId",
    ...
    case (pie::hac::cnmt::ContentMetaType_AddOnContent):
        ...
        r.text(fmt::format("    RequiredApplicationVersion: {:s} (v{:d})",
                           ...
                           mCnmt.getAddOnContentMetaExtendedHeader().getRequiredApplicationVersion()));
        r.text(fmt::format("    ApplicationId:         0x{:016x}",
                           mCnmt.getAddOnContentMetaExtendedHeader().getApplicationId()));

        r.set("data.contentMeta.addOnContentMetaExtendedHeader.requiredApplicationVersion",
              nlohmann::json{{"string", pie::hac::ContentMetaUtil::getVersionAsString(
                                            mCnmt.getAddOnContentMetaExtendedHeader().getRequiredApplicationVersion())},
                             {"int", mCnmt.getAddOnContentMetaExtendedHeader().getRequiredApplicationVersion()}});
        r.set("data.contentMeta.addOnContentMetaExtendedHeader.applicationId",
    ...
    case (pie::hac::cnmt::ContentMetaType_Delta):
        ...
        r.set("data.contentMeta.deltaMetaExtendedHeader.applicationId",
    ...
    if (mCnmt.getContentInfo().size() > 0)
    {
        ...
        for (size_t i = 0; i < mCnmt.getContentInfo().size(); i++)
        {
            ...
            r.text(fmt::format("      Id:           {:s}",
                               tc::cli::FormatUtil::formatBytesAsString(info.getContentId().data(),
                                                                        info.getContentId().size(), false, "")));
            r.text(fmt::format("      Size:         0x{:x}", info.getContentSize()));
            r.text(fmt::format("      Hash:         {:s}",
                               tc::cli::FormatUtil::formatBytesAsString(info.getContentHash().data(),
                                                                        info.getContentHash().size(), false, "")));

            r.push(
                "data.contentMeta.content",
                nlohmann::json{{"type", nlohmann::json{{"string", pie::hac::ContentMetaUtil::getContentTypeAsString(
                                                                      info.getContentType())},
                                                       {"int", info.getContentType()}}},
                               {"id", tc::cli::FormatUtil::formatBytesAsString(info.getContentId().data(),
                                                                               info.getContentId().size(), false, "")},
                               {"size", nlohmann::json{{"hex", fmt::format("0x{:x}", info.getContentSize())},
    ...
    else if (mCnmt.getContentMetaType() == pie::hac::cnmt::ContentMetaType_Delta &&
    ...
    else if (mCnmt.getContentMetaType() == pie::hac::cnmt::ContentMetaType_SystemUpdate &&
             mCnmt.getSystemUpdateMetaExtendedHeader().getExtendedDataSize() != 0)
    {
        ...
        r.set("data.contentMeta.systemUpdateMetadata.formatVersion",
    ...
    r.text(fmt::format("  Digest:   {:s}", tc::cli::FormatUtil::formatBytesAsString(
                                               mCnmt.getDigest().data(), mCnmt.getDigest().size(), false, "")));
    r.set("data.contentMeta.digest",
          tc::cli::FormatUtil::formatBytesAsString(mCnmt.getDigest().data(), mCnmt.getDigest().size(), false, ""));
}

void nstool::CnmtProcess::displayContentMetaInfo(const pie::hac::ContentMetaInfo &content_meta_info,
