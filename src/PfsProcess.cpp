...
nstool::PfsProcess::PfsProcess()
    : mModuleName("nstool::PfsProcess"), mFile(), mCliOutputMode(), mVerify(false), mPfs(), mFileSystem(), mFsProcess()
{
...
void nstool::PfsProcess::process()
{
    ...
    mFileSystem =
        std::make_shared<tc::io::VirtualFileSystem>(tc::io::VirtualFileSystem(pie::hac::PartitionFsSnapshotGenerator(
            mFile, mVerify ? pie::hac::PartitionFsSnapshotGenerator::ValidationMode_Warn
                           : pie::hac::PartitionFsSnapshotGenerator::ValidationMode_None)));
...
{
    return hdr->st_magic.unwrap() == pie::hac::pfs::kPfsStructMagic ||
           hdr->st_magic.unwrap() == pie::hac::pfs::kHashedPfsStructMagic;
}
