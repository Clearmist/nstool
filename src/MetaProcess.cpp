...
void nstool::MetaProcess::validateAciFromAcid(const pie::hac::AccessControlInfo& aci, const pie::hac::AccessControlInfoDesc& acid)
{
	...
	for (size_t i = 0; i < fs_access.size(); i++)
	{
		...
		if (rightFound == false)
		{
			r.text(fmt::format("[WARNING] ACI/FAC FsaRights: FAIL ({:s} not permitted)", pie::hac::FileSystemAccessUtil::getFsAccessFlagAsString(fs_access[i])));

			r.push("events", nlohmann::json{
				{"severity", "warn"},
				{"message", fmt::format("ACI/FAC FsaRights: FAIL ({:s} not permitted)", pie::hac::FileSystemAccessUtil::getFsAccessFlagAsString(fs_access[i]))}
			});
		}
	}
	...
	for (size_t i = 0; i < aci.getFileSystemAccessControl().getContentOwnerIdList().size(); i++)
	{
		...
		if (rightFound == false)
		{
			r.text(fmt::format("[WARNING] ACI/FAC ContentOwnerId: FAIL (0x{:016x} not permitted)", aci.getFileSystemAccessControl().getContentOwnerIdList()[i]));

			r.push("events", nlohmann::json{
				{"severity", "warn"},
				{"message", fmt::format("ACI/FAC ContentOwnerId: FAIL (0x{:016x} not permitted)", aci.getFileSystemAccessControl().getContentOwnerIdList()[i])}
			});
		}
	}
	...
	for (size_t i = 0; i < aci.getServiceAccessControl().getServiceList().size(); i++)
	{
		...
		if (rightFound == false)
		{
			r.text(fmt::format("[WARNING] ACI/SAC ServiceList: FAIL ({:s}{:s} not permitted)", aci.getServiceAccessControl().getServiceList()[i].getName(), (aci.getServiceAccessControl().getServiceList()[i].isServer()? " (Server)" : "")));

			r.push("events", nlohmann::json{
				{"severity", "warn"},
				{"message", fmt::format("ACI/SAC ServiceList: FAIL ({:s}{:s} not permitted)", aci.getServiceAccessControl().getServiceList()[i].getName(), (aci.getServiceAccessControl().getServiceList()[i].isServer()? " (Server)" : ""))}
			});
		}
	}
	...
	{
		r.text(fmt::format("[WARNING] ACI/KC ThreadInfo/MaxCpuId: FAIL ({:d} not permitted)", aci.getKernelCapabilities().getThreadInfo().getMaxCpuId()));

		r.push("events", nlohmann::json{
			{"severity", "warn"},
			{"message", fmt::format("ACI/KC ThreadInfo/MaxCpuId: FAIL ({:d} not permitted)", aci.getKernelCapabilities().getThreadInfo().getMaxCpuId())}
		});
	}

	if (aci.getKernelCapabilities().getThreadInfo().getMinCpuId() != acid.getKernelCapabilities().getThreadInfo().getMinCpuId())
	{
		r.text(fmt::format("[WARNING] ACI/KC ThreadInfo/MinCpuId: FAIL ({:d} not permitted)", aci.getKernelCapabilities().getThreadInfo().getMinCpuId()));

		r.push("events", nlohmann::json{
			{"severity", "warn"},
			{"message", fmt::format("ACI/KC ThreadInfo/MinCpuId: FAIL ({:d} not permitted)", aci.getKernelCapabilities().getThreadInfo().getMinCpuId())}
		});
	}

	if (aci.getKernelCapabilities().getThreadInfo().getMaxPriority() != acid.getKernelCapabilities().getThreadInfo().getMaxPriority())
	{
		r.text(fmt::format("[WARNING] ACI/KC ThreadInfo/MaxPriority: FAIL ({:d} not permitted)", aci.getKernelCapabilities().getThreadInfo().getMaxPriority()));

		r.push("events", nlohmann::json{
			{"severity", "warn"},
			{"message", fmt::format("ACI/KC ThreadInfo/MaxPriority: FAIL ({:d} not permitted)", aci.getKernelCapabilities().getThreadInfo().getMaxPriority())}
		});
	}

	if (aci.getKernelCapabilities().getThreadInfo().getMinPriority() != acid.getKernelCapabilities().getThreadInfo().getMinPriority())
	{
		r.text(fmt::format("[WARNING] ACI/KC ThreadInfo/MinPriority: FAIL ({:d} not permitted)", aci.getKernelCapabilities().getThreadInfo().getMinPriority()));

		r.push("events", nlohmann::json{
			{"severity", "warn"},
			{"message", fmt::format("ACI/KC ThreadInfo/MinPriority: FAIL ({:d} not permitted)", aci.getKernelCapabilities().getThreadInfo().getMinPriority())}
		});
	}
	...
	for (size_t i = 0; i < syscall_ids.size(); i++)
	{
		...
		{
			r.text(fmt::format("[WARNING] ACI/KC SystemCallList: FAIL ({:s} not permitted)", pie::hac::KernelCapabilityUtil::getSystemCallIdAsString(pie::hac::kc::SystemCallId(i))));

			r.push("events", nlohmann::json{
				{"severity", "warn"},
				{"message", fmt::format("ACI/KC SystemCallList: FAIL ({:s} not permitted)", pie::hac::KernelCapabilityUtil::getSystemCallIdAsString(pie::hac::kc::SystemCallId(i)))}
			});
		}
	}
	...
	for (size_t i = 0; i < aci.getKernelCapabilities().getMemoryMaps().getMemoryMaps().size(); i++)
	{
		...
		if (rightFound == false)
		{
			...
			r.push("events", nlohmann::json{
				{"severity", "warn"},
				{"message", fmt::format("ACI/KC MemoryMap: FAIL ({:s} not permitted)", formatMappingAsString(map))}
			});
		}
	}
	...
	for (size_t i = 0; i < aci.getKernelCapabilities().getMemoryMaps().getIoMemoryMaps().size(); i++)
	{
		...
		if (rightFound == false)
		{
			...
			r.push("events", nlohmann::json{
				{"severity", "warn"},
				{"message", fmt::format("ACI/KC IoMemoryMap: FAIL ({:s} not permitted)", formatMappingAsString(map))}
			});
		}
	}
	...
	for (size_t i = 0; i < aci.getKernelCapabilities().getInterupts().getInteruptList().size(); i++)
	{
		...
		if (rightFound == false)
		{
			r.text(fmt::format("[WARNING] ACI/KC InteruptsList: FAIL (0x{:x} not permitted)", aci.getKernelCapabilities().getInterupts().getInteruptList()[i]));

			r.push("events", nlohmann::json{
				{"severity", "warn"},
				{"message", fmt::format("ACI/KC InteruptsList: FAIL (0x{:x} not permitted)", aci.getKernelCapabilities().getInterupts().getInteruptList()[i])}
			});
		}
	}
	...
	{
		r.text(fmt::format("[WARNING] ACI/KC ProgramType: FAIL ({:d} not permitted)", (uint32_t)aci.getKernelCapabilities().getMiscParams().getProgramType()));

		r.push("events", nlohmann::json{
			{"severity", "warn"},
			{"message", fmt::format("ACI/KC ProgramType: FAIL ({:d} not permitted)", (uint32_t)aci.getKernelCapabilities().getMiscParams().getProgramType())}
		});
	}
	...
	{
		r.text(fmt::format("[WARNING] ACI/KC RequiredKernelVersion: FAIL ({:d}.{:d} not permitted)", aci.getKernelCapabilities().getKernelVersion().getVerMajor(), aci.getKernelCapabilities().getKernelVersion().getVerMinor()));

		r.push("events", nlohmann::json{
			{"severity", "warn"},
			{"message", fmt::format("ACI/KC RequiredKernelVersion: FAIL ({:d}.{:d} not permitted)", aci.getKernelCapabilities().getKernelVersion().getVerMajor(), aci.getKernelCapabilities().getKernelVersion().getVerMinor())}
		});
	}
	...
	{
		r.text(fmt::format("[WARNING] ACI/KC HandleTableSize: FAIL (0x{:x} too large)", aci.getKernelCapabilities().getHandleTableSize().getHandleTableSize()));

		r.push("events", nlohmann::json{
			{"severity", "warn"},
			{"message", fmt::format("ACI/KC HandleTableSize: FAIL (0x{:x} too large)", aci.getKernelCapabilities().getHandleTableSize().getHandleTableSize())}
		});
	}
	...
	for (size_t i = 0; i < misc_flags.size(); i++)
	{
		...
		{
			r.text(fmt::format("[WARNING] ACI/KC MiscFlag: FAIL ({:s} not permitted)", pie::hac::KernelCapabilityUtil::getMiscFlagsBitAsString(pie::hac::kc::MiscFlagsBit(i))));

			r.push("events", nlohmann::json{
				{"severity", "warn"},
				{"message", fmt::format("ACI/KC MiscFlag: FAIL ({:s} not permitted)", pie::hac::KernelCapabilityUtil::getMiscFlagsBitAsString(pie::hac::kc::MiscFlagsBit(i)))}
			});
		}
	}
}
...
void nstool::MetaProcess::displayMetaHeader(const pie::hac::Meta& hdr)
{
	...
	r.text(fmt::format("    ProcessAddressSpace:      {:s}", pie::hac::MetaUtil::getProcessAddressSpaceAsString(hdr.getProcessAddressSpace())));
	...
	r.set("data.metaHeader.flags.processAddressSpace", pie::hac::MetaUtil::getProcessAddressSpaceAsString(hdr.getProcessAddressSpace()));
...
void nstool::MetaProcess::displayAciDescHdr(const pie::hac::AccessControlInfoDesc& acid)
{
	...
	r.text(fmt::format("    Memory Region:         {:s} ({:d})", pie::hac::AccessControlInfoUtil::getMemoryRegionAsString(acid.getMemoryRegion()), (uint32_t)acid.getMemoryRegion()));
	...
	r.set("data.accessControl.flags.memoryRegion", nlohmann::json{
		{"string", pie::hac::AccessControlInfoUtil::getMemoryRegionAsString(acid.getMemoryRegion())},
		{"int", (uint32_t)acid.getMemoryRegion()}
	});
	r.set("data.accessControl.programIdRestriction", nlohmann::json{
...
void nstool::MetaProcess::displayFac(const pie::hac::FileSystemAccessControl& fac)
{
	...
	if (fac.getFsAccess().size())
	{
		...
		for (auto itr = fac.getFsAccess().begin(); itr != fac.getFsAccess().end(); itr++)
		{
			...
			r.push("data.fsAccessControl.fsAccess", nlohmann::json{
				{"string", flag_string},
				{"int", (uint32_t)*itr}
			});
		}
	...
	if (fac.getSaveDataOwnerIdList().size())
	{
		...
		{
			r.text(fmt::format("    0x{:016x} ({:s})", fac.getSaveDataOwnerIdList()[i].id, pie::hac::FileSystemAccessUtil::getSaveDataOwnerAccessModeAsString(fac.getSaveDataOwnerIdList()[i].access_type)));

			r.push("data.fsAccessControl.saveDataOwnerIds", nlohmann::json{
				{"string", pie::hac::FileSystemAccessUtil::getSaveDataOwnerAccessModeAsString(fac.getSaveDataOwnerIdList()[i].access_type)},
...
void nstool::MetaProcess::displaySac(const pie::hac::ServiceAccessControl& sac)
{
	...
	for (size_t i = 0; i < sac.getServiceList().size(); i++)
	{
		...
		r.push("data.serviceAccessControl.services", nlohmann::json{
			{"name", sac.getServiceList()[i].getName()},
...
void nstool::MetaProcess::displayKernelCap(const pie::hac::KernelCapabilityControl& kern)
{
	...
	if (kern.getThreadInfo().isSet())
	{
		...
		r.set("data.kernelCapabilities.threadPriority", nlohmann::json{
			{"min", threadInfo.getMinPriority()},
			{"max", threadInfo.getMaxPriority()}
		});
		r.set("data.kernelCapabilities.cpuId", nlohmann::json{
			{"min", threadInfo.getMinCpuId()},
			{"max", threadInfo.getMaxCpuId()}
		});
	}
	...
	if (kern.getSystemCalls().isSet())
	{
		...
		for (size_t syscall_id = 0; syscall_id < syscall_ids.size(); syscall_id++)
		{
			if (syscall_ids.test(syscall_id)) {
				...
				r.push("data.kernelCapabilities.systemCalls", pie::hac::KernelCapabilityUtil::getSystemCallIdAsString(pie::hac::kc::SystemCallId(syscall_id)));
			}
		}
	...
	{
		r.text(fmt::format("  ProgramType:        {:s} ({:d})", pie::hac::KernelCapabilityUtil::getProgramTypeAsString(kern.getMiscParams().getProgramType()), (uint32_t)kern.getMiscParams().getProgramType()));

		r.push("data.kernelCapabilities.programType", nlohmann::json{
			{"string", pie::hac::KernelCapabilityUtil::getProgramTypeAsString(kern.getMiscParams().getProgramType())},
	...
	{
		r.text(fmt::format("  Kernel Version:     {:d}.{:d}", kern.getKernelVersion().getVerMajor(), kern.getKernelVersion().getVerMinor()));

		r.push("data.kernelCapabilities.version", fmt::format("{:d}.{:d}", kern.getKernelVersion().getVerMajor(), kern.getKernelVersion().getVerMinor()));
	}
	...
	if (kern.getHandleTableSize().isSet())
	{
		...
		r.set("data.kernelCapabilities.handleTableSize", fmt::format("0x{:x}", kern.getHandleTableSize().getHandleTableSize()));
	}
	...
	if (kern.getMiscFlags().isSet())
	{
		...
		for (size_t misc_flags_bit = 0; misc_flags_bit < misc_flags.size(); misc_flags_bit++)
		{
			if (misc_flags.test(misc_flags_bit)) {
				...
				r.push("data.kernelCapabilities.miscellaneousFlags", pie::hac::KernelCapabilityUtil::getMiscFlagsBitAsString(pie::hac::kc::MiscFlagsBit(misc_flags_bit)));
			}
		}
